...
```{r , eval=FALSE, anchor=SRCREF}
# Nee, lass ma'!
# Should the names of the columns with the age brackets contain more than just the age range one needs to remove these:
bracket_col_prefix <- "Age range " # prefix to remove
# bracket_cols_names <- grep(bracket_col_prefix, # col names containing prefix
#                            names(practicetime),
#                            value = TRUE,
#                            fixed = TRUE)
names(practicetime) <- sub(pattern = bracket_col_prefix,
                           replacement = "",
                           names(practicetime))
```
...
```{r}
practicetime_dh <- add_daily_hours(practicetime, 
                                   bracket_idx = age_range_idx, 
                                   bracket_mult = multipliers)

```

These calculations assume that subject A played the entire year they were 26 for an average eight daily hours. If we additionally take into account that leap years last an additional day, we need to include the date-of-birth and the time the subject took part in the investigation. By doing so, we can now account for leap years *and* adjust the last bracket's multiplier to the day.

## A More Sophisticated Approach

```{r}
## add age and days since last birthday:
practicetime <- dplyr::mutate(practicetime,
                              Age = praktikr::days_lived(DOB, 
                                                         Date),
                              DaysSinceLastBday = praktikr::days_since_bday(DOB, 
                                                                            Date))
```
...
complete_yrs_spent_in_bracket <- practicetime$
